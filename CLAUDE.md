...
- 検証のために開発サーバを起動したら、検証が終わった時、必ず開発サーバプロセスをキルしてください。そうしなければ、後続の作業に著しい障害が発生してしまいます。
- 3000番ポート以外で開発サーバを起動させないでください。仮に3000番が埋まっていたら、その3000番を使用しているプロセスをキルしてください。

### 自動シード機能付き開発サーバー

`npm run dev` コマンドが自動的に初期データをチェック・挿入してから開発サーバーを起動するように改善されました：

```bash
# 推奨：自動シードチェック付きで開発サーバー起動
npm run dev
...
**自動実行される処理:**
1. データベースの初期データ存在チェック
2. 初期データがない場合のみシード実行
3. Next.js開発サーバー起動

これにより、開発開始時にカテゴリが選択できない問題を防げます。

## Conventional Commit関連メモ
- Conventional Commit形式でコミットメッセージを作成してPushしてください
- CI/CDパイプラインでコミットメッセージの形式を自動チェックする
- Conventional Commitを使用することで、セマンティックバージョニングと自動CHANGELOGの生成が容易になる

## メモリ管理
- 基本的に、私が指示するまでPushはしないでください。Commitは構いません
...
### 現在のワークツリー状況
プロジェクトでは複数のブランチで並行開発を行うためにワークツリーを使用しています。

#### アクティブなワークツリー
- `/workspace` - メインの作業ディレクトリ（mainブランチ）
- `/workspace/.git/phantom/worktrees/notion-db-connect` - Notion DB連携機能の開発中

#### ワークツリー管理ルール
- **開発中のワークツリーは削除しない**: 継続中の機能開発ブランチのワークツリーは保持する
- **マージ済みブランチのクリーンアップ**: PRでマージされたブランチとワークツリーは削除対象
- **リモート確認**: ブランチ削除前にリモートにプッシュされているか必ず確認する

#### Phantom CLI を使用したワークツリー管理コマンド
```bash
# ワークツリー一覧確認
phantom list

# 新しいワークツリー作成
phantom create [ブランチ名]
...
**注意**: 開発中のブランチ（notion-db-connectなど）は削除しないこと
