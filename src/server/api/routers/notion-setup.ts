	...
	createDatabases: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			try {
				...
				throw new Error(
					"データベースの作成に失敗しました。統合がページに招待されているか確認してください。",
				);
			}
		}),
	...
	setupWithDatabaseCreation: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			try {
				...
				const pageValidation = await notionClient.validatePageAccess(
					input.pageId,
				);

				// Step 2: データベースの自動作成
				const databaseResult = await notionClient.createDatabasesInPage(
					input.pageId,
				);
