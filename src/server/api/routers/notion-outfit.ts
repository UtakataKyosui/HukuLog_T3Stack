...
const createNotionClient = (accessToken: string) => {
	...
	if (!clothingDbId || !outfitsDbId) {
		throw new Error(
			"NOTION_DATABASE_IDS environment variable not configured properly",
		);
	}
			...
			try {
				...
				const outfits = result.results.map((page: any) => {
					...
					return {
						...
						description:
							properties.Description?.rich_text?.[0]?.text?.content || null,
						clothingItemIds:
							properties["Clothing Items"]?.relation?.map(
								(rel: any) => rel.id,
							) || [],
		...
		.query(async ({ input, ctx }) => {
			try {
				...
				const clothingItemIds =
					(properties as any)["Clothing Items"]?.relation?.map(
						(rel: any) => rel.id,
					) || [];
					...
					try {
						...
						if ("properties" in itemPage) {
							...
							clothingItems.push({
								...
								brand:
									(itemProps as any).Brand?.rich_text?.[0]?.text?.content ||
									null,
								color: (itemProps as any).Color?.select?.name || null,
								imageUrl:
									(itemProps as any).Image?.files?.[0]?.external?.url || null,
							});
						}
					} catch (itemError) {
				...
				return {
					...
					description:
						(properties as any).Description?.rich_text?.[0]?.text?.content ||
						null,
					clothingItems,
					...
					tags:
						(properties as any).Tags?.multi_select?.map(
							(tag: any) => tag.name,
						) || [],
					createdAt: page.created_time,
					updatedAt: page.last_edited_time,
