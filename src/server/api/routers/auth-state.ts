...
export const authStateRouter = createTRPCRouter({
   ...
   * 現在の認証状態を取得
   */
  getAuthStatus: protectedProcedure.query(async ({ ctx }): Promise<AuthStatus> => {
    const userId = ctx.session.user.id;

    // ユーザー情報とパスキーの有無を取得
    const [userResult, passkeyResult] = await Promise.all([
      ctx.db.select().from(users).where(eq(users.id, userId)).limit(1),
      ctx.db.select({ count: passkeys.id }).from(passkeys).where(eq(passkeys.userId, userId)).limit(1)
    ]);
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db
        ...
        .set({
          ...
          ...(updateData.authCompletedAt && { authCompletedAt: updateData.authCompletedAt }),
          updatedAt: new Date(),
        })
