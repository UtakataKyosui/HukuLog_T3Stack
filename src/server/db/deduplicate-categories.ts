import { sql, inArray, not, eq } from "drizzle-orm";
...
async function deduplicateCategories() {
	try {
		...
		uniqueCategories.forEach((category, index) => {
			console.log(`  ${index + 1}. "${category.name}" (keeping ID: ${category.minId})`);
		});
		...
		console.log("🔄 Updating clothing items to reference first category IDs...");
		...
		for (const uniqueCategory of uniqueCategories) {
			...
			if (duplicateIdsToUpdate.length > 0) {
				console.log(`  Updating items for "${uniqueCategory.name}" from IDs [${duplicateIdsToUpdate.join(', ')}] to ID ${uniqueCategory.minId}`);
