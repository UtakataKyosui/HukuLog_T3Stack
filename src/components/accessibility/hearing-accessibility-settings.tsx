...
import { Eye, Bell, Vibrate, Monitor, Settings, Info } from "lucide-react";
	...
	const testVisualFeedback = () => {
		const event = new CustomEvent('accessibility-test-notification', {
			detail: {
				type: 'info',
				title: 'テスト通知',
				message: 'これは聴覚障害者向けの視覚的通知のテストです。画面の右上に表示されます。',
			}
		});
		document.dispatchEvent(event);
		...
		<Card>
			...
			<CardContent className="space-y-6">
				...
				<div className="space-y-4">
					<h3 className="flex items-center gap-2 font-medium text-theme-text text-sm">
					...
					<div className="space-y-3">
						<div className="flex items-center justify-between">
							...
							<Button
								size="sm"
								variant={settings.enableVisualNotifications ? "default" : "outline"}
								onClick={() => saveSettings({ enableVisualNotifications: !settings.enableVisualNotifications })}
								className="min-w-16"
							>
						...
						<div className="flex items-center justify-between">
							...
							<Button
								...
								onClick={() => saveSettings({ enableFlashIndicator: !settings.enableFlashIndicator })}
								className="min-w-16"
							>
						...
						<div className="flex items-center justify-between">
							...
							<Button
								...
								onClick={() => saveSettings({ showDetailedMessages: !settings.showDetailedMessages })}
								className="min-w-16"
							>
						...
						<div className="flex items-center justify-between">
							...
							<Button
								size="sm"
								variant={settings.enableKeyboardShortcutFeedback ? "default" : "outline"}
								onClick={() => saveSettings({ enableKeyboardShortcutFeedback: !settings.enableKeyboardShortcutFeedback })}
								className="min-w-16"
							>
				...
				<div className="space-y-4">
					...
					<div className="flex items-center justify-between">
						...
						<Button
							...
							onClick={() => saveSettings({ enableVibration: !settings.enableVibration })}
							className="min-w-16"
				...
				<div className="space-y-4">
					...
					<div className="flex items-center justify-between">
						...
						<Button
							...
							onClick={() => saveSettings({ enableStatusBar: !settings.enableStatusBar })}
							className="min-w-16"
						>
					...
					<div className="grid grid-cols-4 gap-2">
						{[2000, 4000, 6000, 8000].map((duration) => (
							<Button
								key={duration}
								size="sm"
								variant={settings.notificationDuration === duration ? "default" : "outline"}
				...
				<div className="rounded-lg border border-theme-primary bg-theme-primary/10 p-4">
					...
					<ul className="space-y-1 text-theme-text-secondary text-xs">
						<li>• <strong>視覚的通知</strong>: 操作結果や状態変更を画面右上に表示</li>
						<li>• <strong>フラッシュ表示</strong>: 重要な変更時に画面全体が一瞬光る</li>
						<li>• <strong>バイブレーション</strong>: モバイルデバイスで触覚フィードバック</li>
						<li>• <strong>状態表示バー</strong>: 現在のテーマと利用可能な操作を画面下部に表示</li>
						<li>• <strong>詳細メッセージ</strong>: 操作の詳細説明と時刻を表示</li>
					</ul>
				</div>

				{/* 緊急時の操作説明 */}
				<div className="rounded-lg border border-theme-warning bg-theme-warning/10 p-4">
					<h4 className="font-medium text-theme-warning text-sm mb-2">
						🚨 緊急時の操作方法
					</h4>
					<ul className="space-y-1 text-theme-text-secondary text-xs">
						<li>• <strong>Escapeキー3回連続</strong>: 即座にハイコントラストモードに切り替え</li>
						<li>• <strong>Ctrl+Shift+H</strong>: ハイコントラストテーマに即切り替え</li>
						<li>• <strong>Alt+T</strong>: クイックテーマセレクターを開く</li>
