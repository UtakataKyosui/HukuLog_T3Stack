...
export function StorageSettings() {
  ...
  const updateStorageTypeMutation = api.userStorage.updateStorageType.useMutation({
    onSuccess: () => {
  ...
  const updateNotionConfigMutation = api.userStorage.updateNotionConfiguration.useMutation({
    onSuccess: () => {
      setHasUnsavedChanges(false);
      setIsConfiguring(false);
      refetchPreferences();
  ...
  const resetNotionMutation = api.userStorage.resetNotionConfiguration.useMutation({
    onSuccess: () => {
  ...
  const handleNotionConfigChange = (field: keyof typeof notionConfig, value: string) => {
  ...
  const handleResetNotion = () => {
    if (confirm("Notion設定をリセットし、PostgreSQLに切り替えますか？\n\n注意: Notionのデータは削除されませんが、このアプリからはアクセスできなくなります。")) {
      setIsConfiguring(true);
          ...
          <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
            ...
            {storagePreferences?.storageType === "notion" && (
              <div className="mt-2 space-y-1 text-blue-700 text-sm">
                <p>• 服DB: {storagePreferences.notionClothingDatabaseId || "未設定"}</p>
                <p>• コーデDB: {storagePreferences.notionOutfitsDatabaseId || "未設定"}</p>
              </div>
            )}
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-600 text-sm">アプリ内のデータベース（推奨）</p>
              </div>
              {selectedStorage === "postgresql" && (
          ...
          >
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-600 text-sm">自分のNotionワークスペース</p>
              </div>
              {selectedStorage === "notion" && (
            ...
            {selectedStorage === "notion" && (
              <div className="mt-4 space-y-4">
                ...
                <div className="space-y-3 border-t pt-4">
                  <div>
                    <Label htmlFor="notion-token-settings">アクセストークン</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="clothing-db-settings">服データベースID</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="outfits-db-settings">コーデデータベースID</Label>
                    <Input
          ...
          <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4">
            <h4 className="mb-2 font-medium text-yellow-800 text-sm">⚠️ 重要な注意事項</h4>
            <ul className="space-y-1 text-yellow-700 text-xs">
              <li>• データ保存方法を変更しても、既存のデータは自動的に移行されません</li>
              <li>• PostgreSQL→Notionに変更した場合、既存の服・コーデデータは表示されなくなります</li>
              <li>• Notion→PostgreSQLに変更した場合も同様です</li>
              <li>• データを移行したい場合は、専用の移行機能をご利用ください</li>
            </ul>
          </div>
        </CardContent>
      </Card>
