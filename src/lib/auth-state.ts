...
export function analyzeAuthStatus(user: UserAuthData): AuthStatus {
  ...
  switch (level) {
    case 1:
      ...
      description = "Passkeyとnotion連携を設定して、セキュリティと利便性を向上させましょう";
      break;
    case 2:
      nextStep = "notion";
      description = "Notion連携を設定して、データを自分のワークスペースで管理しましょう";
      break;
    case 3:
      nextStep = "passkey";
      description = "Passkeyを設定して、より安全で便利なログインを実現しましょう";
      break;
    case 4:
      description = "完全認証が完了しています。最高レベルのセキュリティと利便性をご利用いただけます";
      break;
  }
 ...
 */
export function isFeatureEnabled(authStatus: AuthStatus, feature: string): boolean {
  switch (feature) {
    case "basic_clothing":
    case "basic_outfits":
...
export function calculateAuthUpdate(
  ...
  return {
    ...
    ...(newStatus.level === 4 && currentAuth.authLevel !== 4 && {
